<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.jiankangshipin" label="健康视频" prop="jiankangshipin">
          <file-upload
          tip="点击上传健康视频"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.jiankangshipin?ruleForm.jiankangshipin:''"
          @change="jiankangshipinUploadChange"
          ></file-upload>
        </el-form-item> 
        <div v-else>
          <el-form-item v-if="ruleForm.jiankangshipin" label="健康视频" prop="jiankangshipin">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.jiankangshipin)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.tupian = true;
          ...
          if(o=='jiankangshipin'){
            this.ruleForm.jiankangshipin = obj[o];
	    this.ro.jiankangshipin = true;
            continue;
          }
	...
	this.ruleForm.wenzhangneirong = this.ruleForm.wenzhangneirong.replace(reg,'../../../ssm4pg65/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.jiankangshipin!=null) {
		this.ruleForm.jiankangshipin = this.ruleForm.jiankangshipin.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiankangbaike/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiankangbaikeCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    jiankangshipinUploadChange(fileUrls) {
	this.ruleForm.jiankangshipin = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
