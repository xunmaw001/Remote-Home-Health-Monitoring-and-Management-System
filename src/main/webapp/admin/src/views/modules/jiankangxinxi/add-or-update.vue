<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="数据状态" prop="shujuzhuangtai">
          <el-select :disabled="ro.shujuzhuangtai" v-model="ruleForm.shujuzhuangtai" placeholder="请选择数据状态">
            <el-option
                v-for="(item,index) in shujuzhuangtaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="数据状态" prop="shujuzhuangtai">
	      <el-input v-model="ruleForm.shujuzhuangtai"
                placeholder="数据状态" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xuezhi = true;
          ...
          if(o=='shujuzhuangtai'){
            this.ruleForm.shujuzhuangtai = obj[o];
	    this.ro.shujuzhuangtai = true;
            continue;
          }
        }
      }
      ...
      this.$http({
		...
		if(json.shengao!=''&&json.shengao){
                this.ruleForm.shengao = json.shengao
	    		this.ro.shengao = true;
		}
		if(json.tizhong!=''&&json.tizhong){
                this.ruleForm.tizhong = json.tizhong
	    		this.ro.tizhong = true;
		}
		if(this.$storage.get("role")!="管理员") {
          ...
          this.$message.error(data.msg);
            ...
            this.shujuzhuangtaiOptions = "正常,异常".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
