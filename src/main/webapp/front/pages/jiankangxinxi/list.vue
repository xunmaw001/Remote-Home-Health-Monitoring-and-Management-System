<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.shujuzhuangtai" type="text" placeholder="数据状态" ></input>
			</view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.shujuzhuangtai="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.shujuzhuangtai){
			...
			async search(){
				...
				if(this.searchForm.shujuzhuangtai){
					searchForm['shujuzhuangtai'] = '%' + this.searchForm.shujuzhuangtai + '%'
				}
                let res = {};
                if(this.userid) {
