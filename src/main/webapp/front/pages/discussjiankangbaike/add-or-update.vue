	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussjiankangbaike`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussjiankangbaike`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussjiankangbaike`, this.ruleForm);
						}else{
							await this.$api.add(`discussjiankangbaike`, this.ruleForm);
						}
						uni.setStorageSync('discussjiankangbaikeCleanType',true);
