...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussjiankangbaikeCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussjiankangbaikeCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussjiankangbaike');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussjiankangbaike');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussjiankangbaike', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussjiankangbaike', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussjiankangbaike/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussjiankangbaike', this.detail);
				this.$utils.msg('审核成功');
