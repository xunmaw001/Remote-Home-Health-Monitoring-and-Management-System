		...
		<view class="container">
      	...
      	<swiper-item :style='{"padding":"0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"300rpx"}' v-for="(swiper,index) in swiperList" :key="index">
            ...
            <view style="text-align: center;padding: 20upx;box-sizing: border-box;background: #FFFFFF;" :style='{"padding":"10rpx","boxShadow":"0 0 4rpx rgba(0,0,0,.3)","margin":"0","borderColor":"#fff","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
				<video id="myVideo" :src="baseUrl+detail.jiankangshipin" controls></video>
			</view>
...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			this.getThumbsup();
			let cleanType = uni.getStorageSync('discussjiankangbaikeCleanType')
			if(cleanType){
				uni.removeStorageSync('discussjiankangbaikeCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','jiankangbaike');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','jiankangbaike');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('jiankangbaike', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussjiankangbaike', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussjiankangbaike/add-or-update?refid=${this.id}`)
			},
			// 获取赞踩
			async getThumbsup() {
				let params = {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('jiankangbaike', _this.detail);
								_this.$utils.msg('取消成功');
			...
			async onFinishTap() {
				...
				await this.$api.update('jiankangbaike', this.detail);
				this.$utils.msg('审核成功');
